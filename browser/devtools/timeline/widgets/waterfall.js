...
Waterfall.prototype = {
   ...
   */
  setData: function(markers, startTime, endTime) {
    this.clearView();
    ...
    this._buildHeader(this._headerContents, startTime - markers.startTime, dataScale);
    this._buildMarkers(this._listContents, markers, startTime, endTime, dataScale);
    this.selectRow(this._selectedRow);
  },
  ...
  setupKeys: function() {
    this._document.addEventListener("keydown", e => {
      if (e.keyCode === Ci.nsIDOMKeyEvent.DOM_VK_UP) {
        e.preventDefault();
        this.selectNearestRow(this._selectedRow - 1);
      }
  ...
  _buildMarkers: function(parent, markers, startTime, endTime, dataScale) {
    ...
    for (let marker of markers) {
      ...
      if (rowsCount++ < TIMELINE_IMMEDIATE_DRAW_MARKERS_COUNT) {
  ...
  selectRow: function(idx) {
    ...
    // Unselect
    let prev = this._listContents.querySelector(".timeline-marker-container.selected");
    if (prev) {
  ...
  selectNearestRow: function(idx) {
    ...
    if (idx >= this._listContents.children.length) {
      return this.selectNearestRow(idx - 1);
    }
    let row = this._listContents.children[idx];
    if (row.hasAttribute("is-spacer")) {
      return this.selectNearestRow(idx - 1);
    }
    this.selectRow(idx);
  },
